...
router.post("/remainingStock-report", async (req, res) => {
  try {
...
router.post("/remainingStockPdf-report", async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=Remaining_Stock_Report.pdf"
    );
    ...
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text("Remaining Stock Report", 80, 25)
      .fontSize(10)
      .font("Helvetica")
      .text(`Generated: ${new Date().toLocaleDateString()}`, 80, 45);

    // Column settings - matching stock-in style but with remaining stock columns
